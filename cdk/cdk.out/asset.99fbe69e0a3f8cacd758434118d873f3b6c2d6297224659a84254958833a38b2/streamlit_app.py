...
def run_analysis(local_path: str, source: VideoSource) -> ComplianceResult:
    ...
    try:
        ...
        s3_key = storage.upload_video_to_s3(local_path, request_id)
...
with tab_upload:
    ...
    if uploaded_file and st.button("Analyze Video", key="btn_upload"):
        ...
        result = run_analysis(local_path, VideoSource.UPLOAD)
...
with tab_url:
    ...
    if url_input and st.button("Analyze Video", key="btn_url"):
        with st.spinner("Downloading video..."):
            try:
                local_path, meta = video_input.download_url(url_input)
                if meta.get("title"):
                    st.info(f"Downloaded: **{meta['title']}** ({meta.get('duration', '?')}s)")
            except Exception as e:
                st.error(f"Download failed: {e}")
                local_path = None

        if local_path:
            source = VideoSource.YOUTUBE if "youtu" in url_input else VideoSource.DIRECT_URL
            result = run_analysis(local_path, source)
